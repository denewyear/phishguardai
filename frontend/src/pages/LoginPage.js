...
export default function LoginPage() {
  ...
  const inp = { width: "100%", padding: "11px 13px", background: T.bg, border: `1px solid ${T.border}`, borderRadius: 9, color: T.white, fontSize: 14, outline: "none", boxSizing: "border-box" };
    ...
    <div style={{ minHeight: "calc(100vh - 54px)", display: "flex", alignItems: "center", justifyContent: "center", background: T.bg }}>
      <div style={{ background: T.panel, border: `1px solid ${T.border}`, borderRadius: 16, padding: "42px 40px", width: 400 }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: T.white, marginBottom: 5 }}>Welcome back</div>
          <div style={{ fontSize: 13, color: T.slate }}>Log in to your PhishGuard account</div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input style={{ ...inp, marginBottom: 16 }} type="email" value={email} onChange={e => setEmail(e.target.value)} required placeholder="you@example.com" />
          <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: T.slate, marginBottom: 6 }}>Password</label>
          <input style={{ ...inp, marginBottom: 22 }} type="password" value={password} onChange={e => setPassword(e.target.value)} required placeholder="••••••••" />
        ...
        <div style={{ fontSize: 12, color: T.slate, textAlign: "center", marginTop: 20 }}>
          Don't have an account? <Link to="/register" style={{ color: T.teal, fontWeight: 700 }}>Sign up</Link>
        </div>
      </div>
    </div>
  );
